(() => {
  ...
  loadHistogram = (est) => {
    d3.select('.hist_btn1').on('click', () => {
      ...
      updateHistogram(data, 'Company distribution by employee size', 'Companies', 'Employee size');
    });

    d3.select('.hist_btn2').on('click', () => {
      let thisBtn = d3.select(d3.event.target);
      if (thisBtn.classed('active')) return;
      ...
      updateHistogram(data, 'Employee Distribution', 'Employee Size Category', 'Percentage', true);
    });
  ...
  updateHistogram = (data, title, xLabel, yLabel, isPercent = false) => {
  ...
  updateCardContainer = (est) => {
    ...
    d3
      .select('.card-container .row')
      .selectAll('div').remove();

    let cardContainer = d3
      .select('.card-container .row')
      .selectAll('div')
      .data(data)
      .enter();
    let cardBody = cardContainer
      ...
      cardBody
      .append('div')
      .classed('card-text float-right', true)
      .text(d =>{ return toCommas(d.body) });
